...
async function systemRoutes(fastify, vectorEngine, embeddingService, storageService) {
            ...
            reply.send({
                ...
                stats: {
                    ...
                    blockchain: {
                        ...
                        hasWallet: !!config_1.config.zg.privateKey,
                        contracts: {
                            vectorRegistry: config_1.config.contracts.vectorRegistry,
                            storageOracle: config_1.config.contracts.storageOracle,
                        },
                    },
                },
            });
    ...
    fastify.get('/config', async (request, reply) => {
        reply.send({
            ...
            config: {
                vector: {
                    dimension: config_1.config.vector.dimension,
                    hnsw: config_1.config.vector.hnsw,
                },
                zg: {
                    chainId: config_1.config.zg.chainId,
                    storageUrl: config_1.config.zg.storageUrl,
                    indexerUrl: config_1.config.zg.indexerUrl,
                },
                server: {
    ...
    fastify.post('/test/embedding', async (request, reply) => {
        try {
    ...
    // Test 0G Storage
    fastify.post('/test/storage', async (request, reply) => {
        try {
    ...
    fastify.post('/test/similarity', async (request, reply) => {
        try {
            ...
            const similarity = embeddingService.calculateSimilarity(embedding1.vector, embedding2.vector);
            reply.send({
                success: true,
                similarity: {
                    score: similarity,
    ...
    fastify.post('/test/document', async (request, reply) => {
        try {
            const { content, chunkSize, metadata } = request.body;
            if (!content) {
                return reply.status(400).send({ error: 'Content is required' });
            }
            const chunks = await embeddingService.processDocument(content, metadata || {}, chunkSize || 1000);
            reply.send({
                success: true,
                document: {
