...
class GeminiService {
    constructor(apiKey) {
        this.model = 'gemini-2.5-flash'; // Using the latest available model
     ...
     */
    async generateAnswer(query, context, temperature = 0.7) {
        if (!this.apiKey) {
        ...
        try {
            ...
            // Call Gemini API
            const response = await axios_1.default.post(`${this.apiUrl}/${this.model}:generateContent?key=${this.apiKey}`, {
                contents: [
                    {
                        parts: [
                            {
