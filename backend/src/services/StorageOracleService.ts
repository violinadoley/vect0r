 ...
 */
export class StorageOracleService {
  ...
  constructor() {
    ...
    this.contractAddress = config.contracts.storageOracle;
  ...
  async registerDataStorage(
    ...
    const hashToUse = integrityHash || ethers.keccak256(ethers.toUtf8Bytes(JSON.stringify({
      root,
      nodeAddresses,
      size,
      timestamp: Date.now()
    ...
    try {
      const tx = await this.contract.registerDataStorage(
        root,
        nodeAddresses,
        size,
        hashToUse
   ...
   */
  async removeDataStorage(root: string): Promise<string> {
    if (!this.contract || !this.wallet) {
      throw new Error('StorageOracle contract or wallet not configured');
    }
   ...
   */
  async getStorageEntry(root: string): Promise<StorageEntryInfo | null> {
    if (!this.contract) {
   ...
   */
  async registerNode(nodeAddress: string, totalStorage: number): Promise<string> {
    if (!this.contract || !this.wallet) {
      throw new Error('StorageOracle contract or wallet not configured');
    }
   ...
   */
  async getStorageNode(nodeAddress: string): Promise<StorageNodeInfo | null> {
    if (!this.contract) {
      throw new Error('StorageOracle contract not configured');
    }
   ...
   */
  async getActiveRoots(): Promise<string[]> {
    if (!this.contract) {
      throw new Error('StorageOracle contract not configured');
    }
  ...
  async getStorageStats(): Promise<StorageStats> {
    ...
    try {
      const [totalStoredData, totalActiveNodes, totalActiveRoots, minReplicationFactor] = 
        await this.contract.getStorageStats();
   ...
   */
  async hasSufficientReplication(root: string): Promise<boolean> {
    if (!this.contract) {
      return false;
    }
   ...
   */
  async verifyIntegrity(root: string, nodeAddress: string, actualHash: string): Promise<string> {
    if (!this.contract || !this.wallet) {
      throw new Error('StorageOracle contract or wallet not configured');
    }
   ...
   */
  async requestReplication(root: string, targetReplicas: number): Promise<string> {
    if (!this.contract || !this.wallet) {
      throw new Error('StorageOracle contract or wallet not configured');
    }
  ...
  async getNetworkHealth(): Promise<{
    ...
    activeNodes: string[];
    activeRoots: string[];
    replicationHealth: { root: string; sufficient: boolean; }[];
  }> {
    try {
      ...
      const replicationHealth = await Promise.all(
        recentRoots.map(async (root) => ({
          root,
