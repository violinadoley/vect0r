...
export async function ragRoutes(
   ...
   */
  fastify.post('/rag/query', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      } = request.body as RAGQueryRequest;
      ...
      const searchResults = await vectorEngine.searchVectors(
        collectionId,
   ...
   */
  fastify.get('/rag/status', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
